'''
    BART generation with copy mechanism modified from https://huggingface.co/transformers/_modules/transformers/models/bart/modeling_bart.html
'''
import torch
...
class CopyConditionalGeneration(BartForConditionalGeneration):
    ...
    def forward(
        ...
        cross_attention_non_linear_sum = cross_attention_non_linear.view(self.config.decoder_attention_heads,
                                                                         -1).abs().sum(1)  # (num_heads)
        ...
        last_cross_attentions = outputs.cross_attentions[
        ...
        cross_attentions_aggregate = last_cross_attentions[:, self.selected_heads, :, :].mean(
            dim=1)  # (batch, decoding_seq_length, encoding_seq_length)
        ...
        p_gen = torch.bmm(decoder_last_hidden_state,
